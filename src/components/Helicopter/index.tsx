import { Euler, MathUtils, Quaternion, Raycaster, Vector3 } from "three";
...
export default function Helicopter() {
  ...
  const { hoverForce, maxVelocity, hoverHeight, enable, forward } = useControls(
    "helicopter",
    {
      hoverForce: {
        value: 0.02,
  ...
  useFrame(() => {
    ...
    if (hit) {
      ...
      if (enable) {
        currentHoverHeight.current = MathUtils.lerp(
          currentHoverHeight.current,
          hoverHeight,
          0.01
        );
      } else {
      ...
      const diff =
        currentHoverHeight.current - point.distanceTo(ray.origin as Vector3);
    ...
    // movement
    if (forward) {
      rigidBody.applyImpulse(forwardVector.clone().multiplyScalar(0.04), true);
    }
    const angle =
      0.15 -
      forwardVector.angleTo(
        _linearVelocity.copy(rigidBody.linvel() as Vector3)
      ) *
        0.1;
    const q = new Quaternion().setFromEuler(new Euler(angle, 0, 0));
